...
struct SpotLightBlockGPU {
    ...
    void SetCone(float inner, float outer) { m_innerCone = glm::cos(glm::radians(inner)); m_outerCone = glm::cos(glm::radians(outer)); } // input values in degrees, stored as cosine
